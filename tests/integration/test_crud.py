...
from app.models import ScanBatch, ScanImage, ProcessingStatus
...
def test_get_user_batches_image_count_and_first_image(db_session):
    """The N+1-free path still returns correct image_count + first_image_url (#29)."""
    user, _ = _make_user(db_session)
    batch = ScanBatch(user_id=user.id, status=ProcessingStatus.COMPLETED,
                       total_seeds=4, bad_seeds_count=1)
    db_session.add(batch)
    db_session.flush()
    # Two images; the earliest (image_0) should drive first_image_url.
    db_session.add_all([
        ScanImage(batch_id=batch.id, storage_path=f"uploads/batches/{batch.id}/image_0.png"),
        ScanImage(batch_id=batch.id, storage_path=f"uploads/batches/{batch.id}/image_1.png"),
    ])
    db_session.commit()

    batches, total = crud.get_user_batches(db_session, user.id)
    row = next(b for b in batches if b["id"] == batch.id)
    assert row["image_count"] == 2
    assert row["first_image_url"] == f"/api/images/{batch.id}/image_0.png"


def test_get_user_batches_batch_with_no_images(db_session):
    """A batch with zero images reports image_count 0 and no first_image_url."""
    user, _ = _make_user(db_session)
    batch = ScanBatch(user_id=user.id, status=ProcessingStatus.COMPLETED, total_seeds=0, bad_seeds_count=0)
    db_session.add(batch)
    db_session.commit()
    batches, _ = crud.get_user_batches(db_session, user.id)
    row = next(b for b in batches if b["id"] == batch.id)
    assert row["image_count"] == 0
    assert row["first_image_url"] is None

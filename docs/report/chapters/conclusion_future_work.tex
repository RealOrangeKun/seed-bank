...
versioned and swapped independently, because detection and
...
ML platform trustworthy: an offline evaluation score means
...
\cite{celeryinplay}, the \texttt{ModelResolver} selects a model for the segment, the
...
\subsection{A model registry with production-model resolution and offline evaluation}
...
Only staging and production models can serve traffic. The \texttt{ModelResolver}
decides which model runs for a given segment: it resolves the production model
promoted for that crop type, falling back to a global production model when no
per-type model exists, so a single promoted model can serve every scan. That
fallback is what lets the mobile point-and-shoot flow, which sends no crop type,
still resolve to a model.
...
matrix, persists its metrics to PostgreSQL and writes a Markdown report to
object storage. The model manager keeps a per-process LRU cache of
...
and promote them to production so they serve live scans without a restart.
...
with no churn in the services that call them. The same payoff showed up in model
selection: collapsing the routing layer down to a deterministic, stateless
\texttt{ModelResolver} that resolves the promoted production model for a segment,
with a global fallback, left the services that call it untouched and removed an
entire stateful subsystem from the request path.
...
\texttt{ModelResolver} already resolves models per crop type, so a new classifier
can be promoted and swapped in isolation without disturbing the existing crops. On the data
...
history and the traceability record once connectivity returns.
...
the improved model to production through the registry's existing
staging-to-production lifecycle once it clears that bar. Each turn of the loop
targets generation at the platform's measured weaknesses instead of producing
more of what it already handles well. The registry, the offline-evaluation
runner, the \texttt{ModelResolver}, and MultiSeedGen's reproducible recipes are
all already in place; what remains is the orchestration that connects them.

...
The build view is split into two: \autoref{fig:deployment-build-stages} shows
the multi-stage Dockerfile's 4 builder stages feeding their 4 matching runtime
stages; \autoref{fig:deployment-build-images} shows which runtime stage each
of the three Compose images is built from. The runtime view is split into
three: \autoref{fig:deployment-runtime-app} shows the three containers the
default profile runs; \autoref{fig:deployment-runtime-datastores} shows those
same containers wired to the stateful datastores; \autoref{fig:deployment-runtime-tooling}
shows the same containers alongside the opt-in dev/obs/frontend tooling
profiles.

\figslot[1.1]{10-deployment-build-stages.pdf}{Build view: the multi-stage Dockerfile's 4 builder stages feeding their 4 matching runtime stages.}{fig:deployment-build-stages}

\figslot[1.1]{10-deployment-build-images.pdf}{Build view: the three Compose images (\texttt{api}, \texttt{worker-cpu}, \texttt{worker-inference}) and which Dockerfile runtime stage each is built from.}{fig:deployment-build-images}

\figslot[1.1]{10-deployment-runtime-app.pdf}{Runtime view: the \texttt{api}, \texttt{worker-cpu}, and \texttt{worker-inference} containers the default Compose profile runs, all on the \texttt{seedbank-net} bridge network.}{fig:deployment-runtime-app}

\figslot[1.1]{10-deployment-runtime-datastores.pdf}{Runtime view: the three containers wired to the stateful Postgres/Redis/MinIO/ClickHouse datastores.}{fig:deployment-runtime-datastores}

\figslot[1.1]{10-deployment-runtime-tooling.pdf}{Runtime view: the three containers alongside the opt-in dev (Adminer/ch-ui), obs (Prometheus/Grafana), and frontend tooling profiles.}{fig:deployment-runtime-tooling}
...
\autoref{fig:detector-backbone} shows the shared feature-extraction backbone;
\autoref{fig:detector-head} shows the detection stage that consumes those
feature maps to produce the final class label and bounding box.

\figslot[1.1]{17-fasterRCNN-backbone.pdf}{The Faster R-CNN feature-extraction backbone, showing the ResNet backbone feeding the FPN and PANet to produce the multi-scale feature maps passed to the detection stage.}{fig:detector-backbone}

\figslot[1.1]{17-fasterRCNN-detection-head.pdf}{The Faster R-CNN detection stage, showing the RPN and ROI head consuming the backbone's feature maps to produce the final class label and bounding box.}{fig:detector-head}
...
The EfficientNet-B2 backbone and CBAM attention block is split into two views
so each stays readable at print size. \autoref{fig:classifier-backbone} shows
the backbone feeding the channel attention block; \autoref{fig:classifier-spatial}
shows the spatial attention block combining with the residual identity path;
\autoref{fig:classifier-head} shows the hybrid pooling head that turns the
refined feature map into the final defect logits.

\figslotlarge[1.1]{18-efficientnet-backbone-channel.pdf}{The EfficientNet-B2 backbone feeding the CBAM channel attention block.}{fig:classifier-backbone}

\figslotlarge[1.1]{18-efficientnet-spatial-residual.pdf}{The CBAM spatial attention block, combining the channel-refined feature map with the residual identity path before the hybrid pooling head.}{fig:classifier-spatial}

\figslotlarge[1.1]{18-efficientnet-head.pdf}{The hybrid pooling classification head, combining global average and max pooling on the CBAM-refined feature map before dropout and a final linear projection to defect logits.}{fig:classifier-head}
...
\figslotlarge[1.1]{16-model-resolution.pdf}{Model resolution: the resolver selects the model

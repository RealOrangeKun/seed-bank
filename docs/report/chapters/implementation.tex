...
offline evaluation of a model against a held-out set so we can compare versions
before putting one into use \cite{mlflow2018}, and it runs the actual inference
in a background worker so that uploading a batch of images does not block the
rest of the application while the models run. This part stayed simple on purpose,
since the prototype only needs to serve and compare a handful of models.

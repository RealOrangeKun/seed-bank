...
\begin{itemize}
  ...
  \item Give the ML side real tooling: a model registry with a promotion
        lifecycle, weighted A/B traffic splits, and offline evaluation of any
        model against labelled datasets.
...
lifecycle, a traffic router that splits inference traffic across models for A/B
tests with a sticky per-user bucket, and an experiment runner that scores models
...
mobile, API), supports weighted A/B traffic splits with sticky per-user
assignment, runs offline evaluation of any registered model against labelled
...
experiments are tracked with MLflow~\cite{mlflow2018}; the web client is
...
\begin{longtable}{p{0.22\textwidth} p{0.70\textwidth}}
  ...
  Async \& ML &
    ...
    Roboflow inference SDK, Pillow and OpenCV (headless), MLflow. \\
  Datastores &
...
its model registry and traffic router, the data warehouse, and the three clients.

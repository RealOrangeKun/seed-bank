...
# ── Documents (LaTeX + Pandoc) ────────────────────────────────────────────────
.PHONY: doc-pdf doc-pdf-local doc-docx doc-docx-local doc-clean
DOC_DIR   ?= docs/report
DOC_NAME  ?= main
DOC_OUT   ?= graduation-project

doc-pdf-local: ## Compile to PDF with a locally-installed TeX Live (no Docker, fast rebuilds).
	@command -v latexmk >/dev/null 2>&1 || { echo "latexmk not found. Install a TeX toolchain first (Fedora):"; echo "  sudo dnf install texlive-scheme-medium texlive-polyglossia texlive-biblatex texlive-tex-gyre biber latexmk"; exit 1; }
	cd $(DOC_DIR) && latexmk -pdf -xelatex -interaction=nonstopmode $(DOC_NAME).tex
	@mv "$(DOC_DIR)/$(DOC_NAME).pdf" "$(DOC_DIR)/$(DOC_OUT).pdf" && echo "PDF successfully generated at $(DOC_DIR)/$(DOC_OUT).pdf"

doc-pdf: ## Compile LaTeX to PDF using texlive Docker image.
	@mkdir -p $(DOC_DIR)
	docker run --rm -v "$(PWD)/$(DOC_DIR):/workdir" -w /workdir texlive/texlive latexmk -pdf -xelatex -interaction=nonstopmode $(DOC_NAME).tex
	@if [ -f "$(DOC_DIR)/$(DOC_NAME).pdf" ]; then \
	   mv "$(DOC_DIR)/$(DOC_NAME).pdf" "$(DOC_DIR)/$(DOC_OUT).pdf"; \
	   echo "PDF successfully generated at $(DOC_DIR)/$(DOC_OUT).pdf"; \
	 else \
	   echo "Error: PDF generation failed"; exit 1; \
	 fi

doc-docx: ## Convert LaTeX to DOCX using pandoc Docker image.
	@mkdir -p $(DOC_DIR)
	docker run --rm -v "$(PWD)/$(DOC_DIR):/data" -w /data pandoc/latex $(DOC_NAME).tex -o $(DOC_OUT).docx
	@echo "DOCX successfully generated at $(DOC_DIR)/$(DOC_OUT).docx"

doc-docx-local: ## Build DOCX with local pandoc (no Docker): PNG diagrams + IEEE citations.
	@command -v pandoc >/dev/null 2>&1 || { echo "pandoc not found:  sudo dnf install pandoc"; exit 1; }
	bash scripts/build_docx.sh

doc-clean: ## Clean auxiliary files generated by LaTeX.
	rm -f $(DOC_DIR)/*.aux $(DOC_DIR)/*.log $(DOC_DIR)/*.out $(DOC_DIR)/*.toc $(DOC_DIR)/*.pdf $(DOC_DIR)/*.docx $(DOC_DIR)/*.fdb_latexmk $(DOC_DIR)/*.fls $(DOC_DIR)/*.bbl $(DOC_DIR)/*.bcf $(DOC_DIR)/*.blg $(DOC_DIR)/*.lof $(DOC_DIR)/*.lot $(DOC_DIR)/*.run.xml $(DOC_DIR)/*.xdv
	rm -f $(DOC_DIR)/chapters/*.aux $(DOC_DIR)/chapters/*.log $(DOC_DIR)/chapters/*.toc

# ── Cleanup ──────────────────────────────────────────────────────────────────
.PHONY: clean
clean: doc-clean ## Remove caches, coverage, build artifacts.
